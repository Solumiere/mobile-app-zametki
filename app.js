...
function load(){
  ...
  s.goals.forEach(g=>{ if(!Array.isArray(g.subtasks)) g.subtasks=[]; if(typeof g.repeat!=='string') g.repeat='none'; if(typeof g.reminder!=='string') g.reminder=''; });
  s.habits.forEach(h=>{ if(typeof h.target!=='number') h.target=0; if(typeof h.reminder!=='string') h.reminder=''; });
  s.name=s.name||'Young';
  s.settings=Object.assign({theme:'dark',accent:'mono',lastReminded:'',pin:''}, s.settings||{});
  s.settings.itemReminded=(s.settings.itemReminded&&typeof s.settings.itemReminded==='object')?s.settings.itemReminded:{};
...
function goalCard(g){ const pr={high:'p-high',mid:'p-mid',low:'p-low'}[g.priority]||'p-mid'; const bits=[];
  if(g.category) bits.push(esc(g.category)); if(g.period) bits.push(esc(g.period)); if(g.repeat&&g.repeat!=='none') bits.push('↻ '+({daily:'каждый день',weekly:'каждую неделю',monthly:'каждый месяц'}[g.repeat]||'повтор'));
...
function goalSheet(goal){ const g=goal||{}; let subs=(g.subtasks||[]).map(s=>({id:s.id,title:s.title,done:!!s.done}));
  openSheet('<h3>'+(goal?'Редактировать цель':'Новая цель')+'</h3>'+
    ...
    '<div class="row2"><label class="fld"><span class="lt">Повтор</span><select id="gRep"><option value="none">Без повтора</option><option value="daily">Каждый день</option><option value="weekly">Каждую неделю</option><option value="monthly">Каждый месяц</option></select></label><label class="fld"><span class="lt">Напоминание</span><input type="time" id="gRem" /></label></div>'+
  ...
  document.getElementById('gRep').value=g.repeat||'none';
  document.getElementById('gRem').value=g.reminder||'';
  ...
  document.getElementById('gSave').onclick=()=>{ const title=document.getElementById('gT').value.trim(); if(!title){ toast('Введите название'); return; }
    const data={title:title,category:document.getElementById('gC').value.trim(),priority:document.getElementById('gP').value,deadline:document.getElementById('gD').value,period:document.getElementById('gPer').value.trim(),repeat:document.getElementById('gRep').value,reminder:document.getElementById('gRem').value,subtasks:subs.filter(s=>s.title.trim()).map(s=>({id:s.id,title:s.title.trim(),done:s.done}))};
    if(data.reminder && 'Notification' in window && Notification.permission==='default'){ Notification.requestPermission(); }
...
function habitSheet(habit){ const h=habit||{};
  openSheet('<h3>'+(habit?'Редактировать привычку':'Новая привычка')+'</h3>'+
    ...
    '<label class="fld"><span class="lt">Напоминание</span><input type="time" id="hRem" /></label>'+
  ...
  document.getElementById('hT').value=h.title||''; document.getElementById('hTarget').value=h.target||0; document.getElementById('hRem').value=h.reminder||''; document.getElementById('hT').focus();
  document.getElementById('hSave').onclick=()=>{ const title=document.getElementById('hT').value.trim(); if(!title){ toast('Введите название'); return; }
    ...
    const rem=document.getElementById('hRem').value;
    if(rem && 'Notification' in window && Notification.permission==='default'){ Notification.requestPermission(); }
    if(habit){ habit.title=title; habit.target=tg; habit.reminder=rem; } else { state.habits.push({id:uid(),title:title,target:tg,reminder:rem,log:{}}); }
